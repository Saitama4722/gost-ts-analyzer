"""Stage 9.1: deterministic presence check for document purpose signals."""
...
from backend.app.rules.purpose_rules import PURPOSE_SECTION_HEADINGS, PURPOSE_TEXT_PHRASES


def _normalized_purpose_heading_norms() -> frozenset[str]:
    return frozenset(_normalize_heading_text(h) for h in PURPOSE_SECTION_HEADINGS)


def _canonical_purpose_heading(norm: str) -> str:
    for h in PURPOSE_SECTION_HEADINGS:
        if _normalize_heading_text(h) == norm:
            return h
    return norm


def check_document_purpose(structure: dict[str, Any], full_text: str) -> dict[str, Any]:
    """Return ``purpose_check`` from section titles first, then full text phrases.
    ...
    purpose_norms = _normalized_purpose_heading_norms()
    ...
    for sec in sections:
        ...
        if norm in purpose_norms:
            canonical = _canonical_purpose_heading(norm)
            return {
                "purpose_check": {
                    "is_present": True,
    ...
    for phrase in PURPOSE_TEXT_PHRASES:
