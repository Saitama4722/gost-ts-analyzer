"""Stage 9.2: deterministic presence check for scope / application-area signals."""
...
from backend.app.rules.scope_rules import SCOPE_SECTION_HEADINGS, SCOPE_TEXT_PHRASES


def _normalized_scope_heading_norms() -> frozenset[str]:
    return frozenset(_normalize_heading_text(h) for h in SCOPE_SECTION_HEADINGS)


def _canonical_scope_heading(norm: str) -> str:
    for h in SCOPE_SECTION_HEADINGS:
        if _normalize_heading_text(h) == norm:
            return h
    return norm


def check_document_scope(structure: dict[str, Any], full_text: str) -> dict[str, Any]:
    """Return ``scope_check`` from section titles first, then full text phrases.
    ...
    scope_norms = _normalized_scope_heading_norms()
    ...
    for sec in sections:
        ...
        if norm in scope_norms:
            canonical = _canonical_scope_heading(norm)
            return {
                "scope_check": {
                    "is_present": True,
    ...
    for phrase in SCOPE_TEXT_PHRASES:
